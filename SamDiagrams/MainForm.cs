...
namespace SamDiagrams
{
	...
	public partial class MainForm : Form
	{
		...
		void MainFormLoad(object sender, EventArgs e)
		{
            ...
            for (int i = 0; i < k; i++)
            {
                DiagramItem di = new DiagramItem(diagramContainer1, "ITEM" + i.ToString(), new Point(0, 0), new Size(100, 200));
                di.TitleImage = tImg;
                DiagramItemNode cols = new DiagramItemNode("Columns", di);
                cols.AddNode(new DiagramItemNode("id", true, intImg, di));
                cols.AddNode(new DiagramItemNode("type", true, intImg, di));
                cols.AddNode(new DiagramItemNode("nume", di ));
                cols.AddNode(new DiagramItemNode("prenume", di));
                cols.AddNode(new DiagramItemNode("cheie", di ));
                cols.AddNode(new DiagramItemNode("altacheia", di));
                di.Nodes.Add(cols);
            ...
            for (int i = 0; i < k ; i++)
            {
                    ...
                    int y = r.Next(k);
                    if(x!=y)
                        diagramContainer1.LinkOrchestrator.AddLink(diagramContainer1.DiagramItems[x], diagramContainer1.DiagramItems[y] );
            }
		...
		void DiagramContainer1SelectedItemsChanged(object sender, SelectedItemsChangeArgs e)
		{
			...
			foreach(DiagramItem di in e.SelectedItems)
				listBox1.Items.Add(di.Title);
		}
        ...
        {
            if (diagramContainer1.LinkOrchestrator.LinkStyle == LinkStyle.SingleLine)
            {
                diagramContainer1.LinkOrchestrator.LinkStyle = LinkStyle.StreightLines;
            }
            else
            {
                diagramContainer1.LinkOrchestrator.LinkStyle = LinkStyle.SingleLine;
            }
        }
